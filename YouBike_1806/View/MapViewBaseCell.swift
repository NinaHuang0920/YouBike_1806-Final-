...
class MapViewBaseCell: BaseCell {
    ...
    let showNetworkFailAlert = Notification.Name(rawValue: showNetworkFailAlertNotificationKey)
    ...
    override func setupViews() {
        ...
        GetService.sharedInstance.getMapService(setPinToMapCompletion: {
            ...
            Timer.scheduledTimer(withTimeInterval: 2, repeats: false, block: { (_) in
             ...
             self.showNetworkFailAlert(mapNetworkCheck: mapNetworkCheck)
        })
    ...
    func showNetworkFailAlert(mapNetworkCheck: Bool) {
        if mapNetworkCheck == false {
            UIView.animate(withDuration: 1.5, delay: 0.1, usingSpringWithDamping: 1, initialSpringVelocity: 1, options: UIViewAnimationOptions.curveEaseOut, animations: {
                ...
                self.loadingView.removeFromSuperview()
                NotificationCenter.default.post(name: self.showNetworkFailAlert, object: nil)
            }
        }
    }
...
extension MapViewBaseCell: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        view?.rightCalloutAccessoryView = setupMapViewNavigateButton()
        
        return view
    }
    
    func setupMapViewNavigateButton() -> UIButton {
        let navigateButton = UIButton(frame: CGRect(x: 0, y: 0, width: 40, height: 50))
        ...
        buttonImage.tintColor = navigationTitleColor
        navigateButton.setImage(buttonImage.image, for: .normal)
        navigateButton.addTarget(self, action: #selector(handleNavigateFunction), for: .touchUpInside)
